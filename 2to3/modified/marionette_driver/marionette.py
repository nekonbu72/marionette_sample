...
class Marionette(object):
    ...
    def wait_for_port(self, timeout=None):
        ...
        while datetime.datetime.now() - starttime < datetime.timedelta(seconds=timeout):
            ...
            try:
                ...
                data = sock.recv(16)
                data = data.decode('utf-8')  # nekonbu72 +
                if ":" in data:
                    return True
            except socket.error:
                pass
    ...
    def _handle_socket_failure(self):
        ...
        if not self.instance:
            ...
            raise IOError(message.format(returncode=returncode, reason=val)).with_traceback(tb)
        ...
        if in_app:
            ...
            try:
                ...
                if self.instance.runner.returncode is not None:
                    exc, val, tb = sys.exc_info()
                    self.cleanup()
                    raise exc("Requested restart of the application was aborted").with_traceback(tb)
    ...
    def execute_js_script(self, script, script_args=(), async_=True,  # async=True, # nekonbu72 +-
        ...
        body = {"script": script,
                "args": args,
                # "async": async, # nekonbu72 -
                "async": async_,  # nekonbu72 +
